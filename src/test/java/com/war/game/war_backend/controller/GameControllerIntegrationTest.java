...
class GameControllerIntegrationTest {
  ...
  void setUp() {
    ...
    // Generate JWT tokens
    org.springframework.security.core.userdetails.UserDetails userDetails1 = org.springframework.security.core.userdetails.User
        .withUsername(player1.getUsername())
        .password(player1.getPassword())
        .authorities("USER")
        .build();
    jwtToken1 = "Bearer " + jwtTokenUtil.generateToken(userDetails1);
    
    org.springframework.security.core.userdetails.UserDetails userDetails2 = org.springframework.security.core.userdetails.User
        .withUsername(player2.getUsername())
  ...
  void getCurrentTurnInfo_AuthenticatedPlayer_ShouldReturnTurnInfo() throws Exception {
    mockMvc.perform(get("/api/games/{gameId}/current-turn", game.getId())
  ...
  void getCurrentTurnInfo_PlayerNotInGame_ShouldReturnForbidden() throws Exception {
    ...
    org.springframework.security.core.userdetails.UserDetails outsiderDetails = org.springframework.security.core.userdetails.User
        .withUsername(outsider.getUsername())
  ...
  void getCurrentTurnInfo_GameInLobbyStatus_ShouldReturnBadRequest() throws Exception {
    ...
    mockMvc.perform(get("/api/games/{gameId}/current-turn", lobbyGame.getId())
        .header("Authorization", jwtToken1)
        .contentType(MediaType.APPLICATION_JSON))
  ...
  void getCurrentTurnInfo_NoTurnPlayerDefined_ShouldReturnBadRequest() throws Exception {
    ...
    mockMvc.perform(get("/api/games/{gameId}/current-turn", noTurnGame.getId())
        .header("Authorization", jwtToken1)
        .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isBadRequest());
  }
