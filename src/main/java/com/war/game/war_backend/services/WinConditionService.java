    ...
    public boolean checkObjectiveCompletion(Game game, PlayerGame playerGame) {
        ...
        switch (objective.getType()) {
            ...
            case "ELIMINATE_PLAYER":
                completed =
                        checkEliminatePlayer(game, playerGame, objective)
        ...
        if (completed) {
            eventPublisher.publishEvent(
                    new GameOverEvent(
                            this, game, playerGame, "OBJECTIVE_COMPLETED", objectiveDescription));
                            ...
                            .filter(
                                    continent ->
                                            ownedTerritoriesPerContinent
                                                            .get(continent)
                                                            .equals(
                                                                    CONTINENT_TERRITORY_COUNT
                                                                            .getOrDefault(
                                                                                    continent, 0L))
                                                    && CONTINENT_TERRITORY_COUNT.getOrDefault(
                                                                    continent, 0L)
                                                            > 0)
                            .count();
