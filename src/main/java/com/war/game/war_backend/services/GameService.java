...
public class GameService {
    ...
    private void notifyLobbyUpdateWithPlayers(Long lobbyId, List<PlayerGame> currentPlayers) {
        List<com.war.game.war_backend.controller.dto.response.PlayerLobbyDtoResponse> playerDtos =
                currentPlayers.stream()
                        .map(
                                pg ->
                                        new com.war.game.war_backend.controller.dto.response
                                                .PlayerLobbyDtoResponse(
                                                pg.getId(),
    ...
    public Game findCurrentGameForPlayer(Player player) {
        ...
        List<PlayerGame> activeGames =
                playerGameRepository.findByPlayerAndStillInGame(player, true);
    ...
    public Game addPlayerToLobby(Long lobbyId, Player player) {
        ...
        if (currentPlayers.size() >= GameConstants.MAX_PLAYERS) {
            throw new RuntimeException(
                    "Lobby cheio. Número máximo de jogadores alcançado ("
                        ...
                        .orElseThrow(
                                () ->
                                        new RuntimeException(
                                                "Erro interno: Nenhuma cor disponível."));
        ...
        if (activePlayersCount == 1) {
            ...
            PlayerGame winner =
                    playerGameRepository.findByGame(game).stream()
                            ...
                            .findFirst()
                            .orElseThrow(
                                    () -> new RuntimeException("Erro ao determinar vencedor."));
    ...
    public Game startGame(Long gameId, String initiatingUsername) {
        ...
        playerGames.stream()
                ...
                .orElseThrow(
                        () ->
                                new RuntimeException(
                                        "Apenas o dono do lobby pode iniciar a partida."));
        ...
        PlayerGame firstPlayer =
                playerGames.stream()
                        ...
                        .findFirst()
                        .orElseThrow(
                                () -> new RuntimeException("Erro ao definir o primeiro jogador."));
    ...
    public Game allocateTroops(Long gameId, String username, Long territoryId, Integer count) {
        ...
        GameTerritory gameTerritory =
                gameTerritoryRepository
                        ...
                        .orElseThrow(
                                () ->
                                        new RuntimeException(
                                                "Território não encontrado nesta partida."));
        ...
        System.out.println(
                "  - Owner Username: " + gameTerritory.getOwner().getPlayer().getUsername());
        System.out.println("CurrentPlayerGame ID: " + currentPlayerGame.getId());
        System.out.println(
                "IDs iguais? "
                        + gameTerritory.getOwner().getId().equals(currentPlayerGame.getId()));
                    ...
                    PlayerGame firstTurnPlayer =
                            playerGameRepository.findByGame(game).stream()
                                    ...
                                    .orElseThrow(
                                            () ->
                                                    new RuntimeException(
                                                            "Erro ao definir o jogador inicial do jogo."));
                ...
                controlledTerritories.stream()
                        .collect(
                                Collectors.groupingBy(
                                        gt ->
                                                gt.getTerritory()
                                                        .getContinent(), // Assumindo que Territory
                                        // tem getContinent()
                                        Collectors.counting()));
    ...
    public Game startNextTurn(Long gameId, String initiatingUsername) {
        ...
        if (!currentPlayerGame.getPlayer().getUsername().equals(initiatingUsername)) {
            throw new RuntimeException(
                    "Você não tem permissão para encerrar o turno de outro jogador.");
        }
        ...
        if (GameStatus.REINFORCEMENT.name().equals(currentStatus)) {
            ...
            for (GameTerritory territory : allTerritories) {
                if (territory.getMovedInArmies() > 0) {
                    territory.setStaticArmies(
                            territory.getStaticArmies() + territory.getMovedInArmies());
            ...
            if (activePlayers.size() <= 1) {
                ...
                throw new RuntimeException(
                        "Erro de estado do jogo. Nenhum jogador ativo para avançar.");
            }
    ...
    public Game tradeCardsForReinforcements(
        ...
        if (!GameStatus.REINFORCEMENT.name().equals(game.getStatus())) {
            throw new InvalidGamePhaseException(
                    "Só é permitido trocar cartas na fase de reforço. Fase atual: "
                            + game.getStatus(),
                    game.getStatus(),
                    "REINFORCEMENT");
        }
    ...
    public Game attackTerritory(Long gameId, String initiatingUsername, AttackRequestDto dto) {
        ...
        if (!GameStatus.ATTACK.name().equals(game.getStatus())) {
            throw new InvalidGamePhaseException(
                    "Ação inválida. A partida não está na fase de Ataque. Fase atual: "
                            + game.getStatus(),
                    game.getStatus(),
                    "ATTACK");
        }
        ...
        GameTerritory sourceTerritory =
                gameTerritoryRepository
                        ...
                        .orElseThrow(
                                () -> new RuntimeException("Território atacante não encontrado."));
        GameTerritory targetTerritory =
                gameTerritoryRepository
                        .findByGame_IdAndTerritory_Id(gameId, dto.getTargetTerritoryId())
                        .orElseThrow(
                                () -> new RuntimeException("Território defensor não encontrado."));
    ...
    public Game moveTroops(
        ...
        GameTerritory sourceTerritory =
                gameTerritoryRepository
                        ...
                        .orElseThrow(
                                () -> new RuntimeException("Território de origem não encontrado."));
        GameTerritory targetTerritory =
                gameTerritoryRepository
                        ...
                        .orElseThrow(
                                () ->
                                        new RuntimeException(
                                                "Território de destino não encontrado."));
        ...
        if (!isNeighbor) {
            throw new RuntimeException(
                    "O território de destino não é vizinho do território de origem.");
        }
    ...
    private int calculateTerritoryMatchBonus(
        ...
        for (Card card : cardsToTrade) {
            ...
            if (ownedTerritoriesMap.containsKey(territoryMasterId)) {
                ...
                bonus += 2;
                System.out.println(
                        "Bônus de território para a carta: " + card.getTerritory().getName());
            }
        }
        return bonus;
    }
    ...
    public Game findGameById(Long gameId) {
        return gameRepository
                ...
                .orElseThrow(
                        () ->
                                new RuntimeException(
                                        "Partida com ID " + gameId + " não encontrada."));
    }
}
