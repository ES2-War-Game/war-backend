...
public class GameService {
  ...
  public Game addPlayerToLobby(Long lobbyId, Player player) {
    ...
    // Remove o jogador de outros lobbies ativos (status LOBBY) ANTES de verificar se já está no
    // lobby atual
  ...
  public Game allocateTroops(Long gameId, String username, Long territoryId, Integer count) {
    ...
    if (GameStatus.REINFORCEMENT.name().equals(currentStatus)
        && !game.getTurnPlayer().equals(currentPlayerGame)) {
        ...
        controlledTerritories.stream()
            .collect(
                Collectors.groupingBy(
                    gt ->
                        gt.getTerritory()
                            .getContinent(), // Assumindo que Territory tem getContinent()
                    Collectors.counting()));
  ...
  public Game attackTerritory(Long gameId, String initiatingUsername, AttackRequestDto dto) {
    ...
    int[] combatResult =
        resolveCombat(attackRolls, defenseRolls); // [perdas_atacante, perdas_defensor]
    ...
    if (currentStaticArmies >= remainingDefenderLosses) {
      ...
      targetTerritory.setMovedInArmies(
          targetTerritory.getMovedInArmies() - remainingDefenderLosses);
    }

    // Log (manter log para debug)
    System.out.printf(
        "Combate: %s vs %s. Atacante (%s) perdeu %d. Defensor (%s) perdeu %d.\n",
    ...
    if ((targetTerritory.getStaticArmies() + targetTerritory.getMovedInArmies()) <= 0) {
      ...
      if (dto.getTroopsToMoveAfterConquest() < dto.getAttackDiceCount()
          || dto.getTroopsToMoveAfterConquest() >= armiesAvailable) {
        throw new RuntimeException(
            String.format(
                "Movimento de exércitos inválido após a conquista. Mínimo: %d, Máximo: %d.",
                dto.getAttackDiceCount(), armiesAvailable - 1));
      }
      ...
      sourceTerritory.setStaticArmies(
          sourceTerritory.getStaticArmies() - dto.getTroopsToMoveAfterConquest());
  ...
  public void checkGameOver(Game game, PlayerGame defeatedPlayer) {
    ...
    if (territoriesOwned == 0) {
      ...
      if (!defeatedCards.isEmpty()) {
        ...
        System.out.println(
            String.format(
                "Transferidas %d cartas de %s para %s.",
                defeatedCards.size(), defeatedPlayer.getUsername(), attackerPlayer.getUsername()));
      }
